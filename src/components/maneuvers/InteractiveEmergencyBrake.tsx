  ...
  return (
    <div className="flex flex-col gap-6 rounded-3xl bg-[#030712] p-6 border border-[#1e293b] shadow-2xl">
      ...
      <div className="relative aspect-[16/9] w-full overflow-hidden rounded-2xl border border-[#1e293b] bg-[#020617] shadow-inner">
        <svg viewBox="0 0 400 225" className="w-full h-full">
          ...
          <g transform="translate(350, 190)">
            ...
            <text textAnchor="middle" y="5" fill="#38BDF8" className="text-sm font-bold">30</text>
          ...
          {gameState === 'signal' && (
            ...
            >
              <div className="bg-red-600 px-10 py-6 rounded-3xl border-4 border-white shadow-[0_0_50px_rgba(220,38,38,0.5)] animate-pulse">
                <h2 className="text-5xl font-bold text-white italic tracking-tighter uppercase">
                  {et.stop}
                </h2>
              </div>
            </motion.div>
            ...
            >
              {gameState === 'braked' ? (
                <>
                  ...
                  <h3 className="text-2xl font-bold uppercase tracking-tight">{et.success}</h3>
                  <div className="mt-6 flex items-center gap-3 bg-white text-emerald-600 px-6 py-3 rounded-2xl shadow-xl">
                    <Timer className="h-6 w-6" />
                    <span className="font-bold text-3xl">{reactionTime}ms</span>
                  </div>
                  <button 
                    onClick={onComplete}
                    className="mt-8 w-full max-w-xs bg-white text-emerald-600 py-4 rounded-2xl font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-2xl"
                  >
                ...
                <>
                  ...
                  {reactionTime && (
                    <div className="mt-6 flex items-center gap-3 bg-white/20 px-6 py-3 rounded-2xl">
                      ...
                      <span className="font-bold text-2xl">{reactionTime}ms</span>
                    </div>
                  )}
                  <button 
                    onClick={reset}
                    className="mt-8 w-full max-w-xs bg-white text-red-600 py-4 rounded-2xl font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-2xl"
                  >
                    TRY AGAIN
                  </button>
                </>
      ...
      <button
        ...
        className={cn(
          'w-full py-8 rounded-3xl font-bold text-3xl transition-all shadow-2xl active:scale-95 mb-2 uppercase tracking-widest',
          gameState === 'signal'
            ? 'bg-red-600 text-white animate-pulse shadow-[0_0_40px_rgba(220,38,38,0.4)]'
