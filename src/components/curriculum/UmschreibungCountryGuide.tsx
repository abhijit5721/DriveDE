...
const TIER_BADGE: Record<UmschreibungTier, { bg: string; text: string; border: string; dot: string }> = {
  eu:      { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/30', dot: 'bg-emerald-400' },
  annex11: { bg: 'bg-blue-500/10',    text: 'text-blue-400',    border: 'border-blue-500/30',    dot: 'bg-blue-400'    },
  other:   { bg: 'bg-amber-500/10',   text: 'text-amber-400',   border: 'border-amber-500/30',   dot: 'bg-amber-400'   },
};
...
export function UmschreibungCountryGuide({ onClose, standalone = false }: UmschreibungCountryGuideProps) {
  ...
  const wrapper = standalone
    ? 'min-h-screen bg-slate-900 px-4 py-8 md:px-8'
    : 'fixed inset-0 z-[110] flex items-start justify-center overflow-y-auto bg-slate-950/70 p-4 pt-16 backdrop-blur-md md:items-center md:pt-4';

  const card = standalone
    ? 'mx-auto w-full max-w-2xl'
    : 'relative w-full max-w-2xl rounded-3xl border border-slate-700/60 bg-slate-900 p-6 shadow-2xl';
      ...
      <div className={card}>
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="ml-4 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-800 text-slate-400 transition hover:bg-slate-700 hover:text-white"
            >
              <X className="h-4 w-4" />
            </button>
          )}
        ...
        <div className="relative mb-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-700 bg-slate-800 py-3.5 pl-11 pr-11 text-white placeholder-slate-500 outline-none transition focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50"
            />
            {query && (
              <button
                onClick={handleClear}
          ...
          <AnimatePresence>
            {showDropdown && filtered.length > 0 && !selected && (
              <motion.div
                ...
                className="absolute z-20 mt-2 w-full rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl overflow-hidden"
              >
            ...
            >
              ...
              <div className={cn(
                ...
                <div>
                  <p className={cn(
                    'text-sm font-bold',
                    tierConfig.testsRequired ? 'text-amber-300' : 'text-emerald-300'
                  )}>
                    {tierConfig.testsRequired
